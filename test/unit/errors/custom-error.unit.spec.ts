...
const testCustomError = (errorConstructor: Function) => {
	describe(`${errorConstructor.name} - Unit`, () => {
		context('constructor', () => {
			...
			it('should should set innerError property if provided implicitly or explicitly', () => {
				...
				customError = Reflect.construct(errorConstructor, [
					'Error message',
					new Error('Explicit inner error')
				]);

				customError.innerError.message.should.equal('Explicit inner error');

				customError = Reflect.construct(errorConstructor, [
					new Error('Implicit inner error')
				]);
			...
			it('should resolve message based on message property and inner error chain', () => {
				...
				customError.message.should.equal(
					`${errorConstructor.name}: Implicit inner error (Error)`
				);

				customError = Reflect.construct(errorConstructor, [
					'Error message',
					new Error('Inner error message')
				]);
				customError.message.should.equal(
					`${errorConstructor.name}: Error message. Inner error message (Error)`
				);

				customError = Reflect.construct(errorConstructor, [
					'Error message',
					Reflect.construct(errorConstructor, ['Inner error 1', new Error()])
				]);
				customError.message.should.equal(
					`${errorConstructor.name}: Error message. Inner error 1 (${errorConstructor.name})`
				);

				customError = Reflect.construct(errorConstructor, [
					'Error message',
					Reflect.construct(errorConstructor, [
						'Inner error 1',
						new Error('Inner error 2')
					])
				]);
				customError.message.should.equal(
					`${errorConstructor.name}: Error message. Inner error 1 (${errorConstructor.name}). Inner error 2 (Error)`
				);
			});
		});
	});
