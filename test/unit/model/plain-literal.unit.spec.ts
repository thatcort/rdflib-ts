...
describe('PlainLiteral - Unit', () => {
	...
	context('constructor', () => {
		...
		it('should throw InvalidOperationError if sparql query result binding provided and its type is literal with xml:lang and datatype specified', () => {
			...
			() => new PlainLiteral({ type: 'literal', value: 'b1', datatype: 'xsd:string' }).should.throw(InvalidOperationError);
			() => new PlainLiteral({ type: 'literal', value: 'b1', 'xml:lang': 'en' }).should.throw(InvalidOperationError);
		});
	});
