...
describe('NQuad - Unit', () => {
	context('constructor', () => {
		it('should set subject, predicate and object properties', () => {
			let quad = new NQuad(new BlankNode(), 'rdf:knows', {
				type: 'literal',
				value: 'Literal',
				'xml:lang': 'en'
			});
		...
		it('should throw InvalidOperation error if specified invalid graph value', () => {
			...
			(() =>
				new NQuad(
					...
					{ type: 'uri', value: 'rdf:knows' },
					{ type: 'bnode', value: 'b2' },
					{ type: 'literal', value: 'b3', 'xml:lang': 'en' }
				)).should.throw(InvalidOperationError);
			(() =>
				new NQuad(
					new BlankNode(),
					{ type: 'uri', value: 'rdf:knows' },
					{ type: 'bnode', value: 'b2' },
					{ type: 'literal', value: 'b3', datatype: 'xsd:string' }
				)).should.throw(InvalidOperationError);
			(() =>
				new NQuad(
					new BlankNode(),
					{ type: 'uri', value: 'rdf:knows' },
					{ type: 'bnode', value: 'b2' },
					{ type: 'typed-literal', value: 'b3', datatype: 'xsd:string' }
				)).should.throw(InvalidOperationError);
		});
	});
	...
	context('toString', () => {
		it('should return space separated subject, predicate and object and optionally graph with dot at the and', () => {
			...
			quad.toString().should.equal(
				'_:b1 <http://example.org#knows> _:b2 <http://example.org#graph> .'
			);
		});
	});
});
