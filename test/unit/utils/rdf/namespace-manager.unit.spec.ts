...
describe('NamespaceManager - Unit', () => {
	...
	context('constructor', () => {
		it('should add default namespaces (rdf, rdfs, xsd, skolem', () => {
			...
			manager.coreNamespaces.some(n => n.prefix === 'rdf' && n.value === 'http://www.w3.org/1999/02/22-rdf-syntax-ns#').should.be.true;
			manager.coreNamespaces.some(n => n.prefix === 'rdfs' && n.value === 'http://www.w3.org/2000/01/rdf-schema#').should.be.true;
			manager.coreNamespaces.some(n => n.prefix === 'xsd' && n.value === 'http://www.w3.org/2001/XMLSchema#').should.be.true;
			manager.coreNamespaces.some(n => n.prefix === 'skolem' && n.value === 'https://bitbucket.org/vladimir_djurdjevic/rdflib.ts/.well-known/genid/').should.be.true;
		});
	});
	...
	context('getNamespace', () => {
		it('should return null if namespace with prefix or value does not exist', () => {
			should.equal(manager.getNamespace(null, null), null);
			should.equal(manager.getNamespace('un', 'http://unknown.org'), null);
		});

		it('should return namespace if namespace with target prefix or value exists in core or generated namespaces', () => {
			manager.getNamespace('rdf', null).value.should.equal('http://www.w3.org/1999/02/22-rdf-syntax-ns#');
			manager.getNamespace(null, 'http://www.w3.org/1999/02/22-rdf-syntax-ns#').prefix.should.equal('rdf');
	...
	context('getNamespaceByPrefix', () => {
		it('should return null if namespace with prefix does not exist', () => {
			should.equal(manager.getNamespaceByPrefix('aaaa'), null);
		});

		it('should return namespace if namespace with target prefix exists in core or generated namespaces', () => {
			manager.getNamespaceByPrefix('rdf').value.should.equal('http://www.w3.org/1999/02/22-rdf-syntax-ns#');
		});
	});

	context('getNamespaceByValue', () => {
		it('should return null if namespace with value does not exist', () => {
			should.equal(manager.getNamespaceByValue('http://unknownnamespace.org#'), null);
		});

		it('should return namespace if namespace with target value exists in core or generated namespaces', () => {
			manager.getNamespaceByValue('http://www.w3.org/1999/02/22-rdf-syntax-ns#').prefix.should.equal('rdf');
		});
	});
	...
	context('registerNamespace', () => {
		it('should create and add to core namespaces new namespace object if namespace with specified prefix does not exist', () => {
			...
			manager.coreNamespaces.some(n => n.prefix === 'ex' && n.value === 'http://example.org#').should.be.true;
		});
