...
import { SparqlQueryResultBinding } from '../../model/sparql-query-result';
...
export class RdfFactory {
	...
	public static createRdfTermFromSparqlResultBinding(
		binding: SparqlQueryResultBinding
	): RdfTerm {
		switch (binding.type) {
			case 'uri':
				return new IRI(binding.value);
			case 'bnode':
				return new BlankNode(binding.value);
			default:
				return RdfFactory.createLiteral(
					binding.value,
					binding['xml:lang'],
					binding.datatype
				);
